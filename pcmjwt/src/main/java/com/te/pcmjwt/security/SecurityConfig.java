...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable();
		/**
		 * http.authorizeRequests().antMatchers("/auth/**").hasRole("ADMIN");
		 */

		http.authorizeRequests().antMatchers("/auth/employee/**").permitAll().anyRequest().authenticated();

		/**
		 * http.authorizeRequests().antMatchers(HttpMethod.GET,"/auth/**").permitAll().anyRequest().authenticated();
		 */
